...
`))
	...
	assert.Equal(t, []string{"ingest", "report"}, decls[0].Flows, "flow-style list decodes")
	assert.Equal(t, "the pipeline", decls[0].Note)
	assert.Equal(t, []string{"exit", "reconcile"}, decls[1].Flows, "block-style list decodes")
}
...
func TestLoadRejectsBareListDomain(t *testing.T) {
	t.Parallel()

	// A domain must be a {flows:, note:} mapping, not a bare list.
	_, err := Load(write(t, "domains:\n  d: [a, b]\n"))
	require.Error(t, err)
}
